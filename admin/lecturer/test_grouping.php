...
$ul = "";
$child_ul = "";
//$lecture_id = $_POST['lecture_group_id'];
...
$ul = "<ul>\n";
  $sql = "SELECT * 
    ...
    $lecture_group_id = $_SESSION['class_id'];
    ...
    echo heavy_button($staff_number, "staff_number", $lecture_group_id,"lecture_group_id", $module_code, "module_code", "view_class", "Back", "timetable.php", "back_button");
    ...
    if(isset($_POST['submit_num_groups']))
    {
        ...
        if((int)$num_groups  <= 26 && (int)$num_groups > 0 )
        {
            for($x =0; $x < (int)$num_groups; $x++)
            {
                ...
                {
                    $sql = "SELECT * FROM student WHERE stud_number = " . $_SESSION['student_num'][$_array_size + $_remainder_counter - $_tot_reminder];
                    $result = mysqli_query($link, $sql);
                    if(mysqli_num_rows($result)>0)
                    {
                        while($row = mysqli_fetch_assoc($result))
